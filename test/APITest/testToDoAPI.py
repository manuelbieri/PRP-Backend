...
class ToDoAPITest(testAPI.APITest):
    testAPI.APITest.api_base_path = '/api/v1'
    ...
    def test_get_all_todos(self):
        response = self.app.get(ToDoAPITest.api_base_path + '/todos/index', headers=self.headers)
    ...
    def test_get_selected_todos_multiple_results(self):
        response = self.app.get(ToDoAPITest.api_base_path + '/todos?key=title&value=light', headers=self.headers)
        self.assertEqual(2, len(response.json))

    def test_get_selected_todos_single_result(self):
        response = self.app.get(ToDoAPITest.api_base_path + '/todos?key=title&value=Shopping', headers=self.headers)
        self.assertEqual(1, len(response.json))

    def test_get_selected_todos_invalid_key(self):
        response = self.app.get(ToDoAPITest.api_base_path + '/todos?key=invalid&value=Shopping', headers=self.headers)
    ...
    def test_invalid_username_login(self):
        response = self.app.post(ToDoAPITest.api_base_path + '/login?username=invalid&password=adminTest')
        self.assertEqual(200, response.status_code)
        self.assertEqual('Invalid user', response.json.get('message'))

    def test_invalid_password_login(self):
        response = self.app.post(ToDoAPITest.api_base_path + '/login?username=admin&password=invalid')
        self.assertEqual(200, response.status_code)
        self.assertEqual('Invalid credentials', response.json.get('message'))

    def test_valid_auth_required(self):
        response = self.app.get(ToDoAPITest.api_base_path + '/user', headers=self.headers)
        self.assertEqual(200, response.status_code)
        self.assertEqual(1, response.json)

    def test_invalid_auth_required(self):
        response = self.app.get(ToDoAPITest.api_base_path + '/user')
